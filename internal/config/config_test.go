...
func TestLoad_IdempotencyRouteEnvironmentOverrides(t *testing.T) {
	...
	t.Setenv("APP_IDEMPOTENCY_HTTP_PATHS", "[/api/v1/rule-sets/create, /api/v1/rule-versions/publish]")
	t.Setenv("APP_IDEMPOTENCY_GRPC_METHODS", "[/platform.rule.v1.RuleService/CreateRuleSet]")
	cfg, err := Load(path)
	if err != nil {
		t.Fatal(err)
	}
	if got := strings.Join(cfg.Idempotency.HTTPPaths, ","); got != "/api/v1/rule-sets/create,/api/v1/rule-versions/publish" {
		t.Fatalf("HTTPPaths = %q", got)
	}
	if got := strings.Join(cfg.Idempotency.GRPCMethods, ","); got != "/platform.rule.v1.RuleService/CreateRuleSet" {
...
func TestConfig_ApplicationScopeRequiresConfiguredUpstream(t *testing.T) {
	...
	cfg.Database.DSN = "postgres://rule:secret@db.example.test/platform"
