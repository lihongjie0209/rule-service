...
import (
	...
	docs "github.com/lihongjie0209/rule-service/docs"
	...
	"github.com/lihongjie0209/rule-service/internal/ratelimit"
...
func NewServer(lc fx.Lifecycle, cfg config.Config, handler *Handler, authService *auth.Service, authorizer platformauthz.Authorizer, limiter *ratelimit.Limiter, idempotencyManager *idempotency.Manager, metrics *observability.Metrics, tracing *observability.Tracing, logger *slog.Logger) (*http.Server, error) {
	...
	api.POST("/rule-sets/create", handler.CreateRuleSet)
	api.POST("/rule-sets/update", handler.UpdateRuleSet)
	api.POST("/rule-sets/get", handler.GetRuleSet)
	api.POST("/rule-sets/list", handler.ListRuleSets)
