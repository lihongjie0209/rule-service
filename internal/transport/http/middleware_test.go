...
func TestIdempotencyExecutionCompletesAndReplaysRuleMutation(t *testing.T) {
	t.Parallel()
	...
	router.Use(RequestID(), func(c *gin.Context) {
		...
		c.Next()
	}, IdempotencyExecution(manager, []string{"/api/v1/rule-sets/create"}, slog.New(slog.NewTextHandler(io.Discard, nil))))
	router.POST("/api/v1/rule-sets/create", func(c *gin.Context) { calls++; OK(c, gin.H{"id": "rule-1"}) })
	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, httptest.NewRequest(http.MethodPost, "/api/v1/rule-sets/create", strings.NewReader(`{"code":"approval"}`)))
	...
	replay := httptest.NewRequest(http.MethodPost, "/api/v1/rule-sets/create", strings.NewReader(`{"code":"approval"}`))
...
func TestAuthorizationFailsClosedAndClassifiesOutage(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			router.POST("/api/v1/rules/evaluate", func(c *gin.Context) { OK(c, nil) })
			recorder := httptest.NewRecorder()
			router.ServeHTTP(recorder, httptest.NewRequest(http.MethodPost, "/api/v1/rules/evaluate", nil))
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			router.POST("/api/v1/external/callback", func(c *gin.Context) {
				...
				if test.status == http.StatusOK && (!ok || value.ID != "rule-service:psk" || value.Type != platformprincipal.TypeServiceAccount) {
