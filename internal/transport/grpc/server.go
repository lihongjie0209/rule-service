...
import (
	...
	"github.com/lihongjie0209/rule-service/internal/environment"
	apphealth "github.com/lihongjie0209/rule-service/internal/health"
	"github.com/lihongjie0209/rule-service/internal/idempotency"
	"github.com/lihongjie0209/rule-service/internal/observability"
	"github.com/lihongjie0209/rule-service/internal/requestid"
...
func NewServer(lc fx.Lifecycle, cfg config.Config, authService *auth.Service, authorizer platformauthz.Authorizer, healthService *apphealth.Service, ruleService *rule.Service, idempotencyManager *idempotency.Manager, metrics *observability.Metrics, logger *slog.Logger) (*Server, error) {
	...
	rulev1.RegisterRuleServiceServer(grpcServer, &ruleServer{service: ruleService})
...
func authenticateGRPC(ctx context.Context, method string, service *auth.Service, cfg config.Auth) (context.Context, error) {
	...
	if cfg.PSK.Enabled && auth.MatchesAny(method, cfg.PSK.GRPCMethods) {
		...
		return platformprincipal.WithContext(ctx, platformprincipal.Principal{ID: "rule-service:psk", Type: platformprincipal.TypeServiceAccount}), nil
	}
