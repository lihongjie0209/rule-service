...
func (s *Service) CreateRuleSet(ctx context.Context, tenantID, code, name, description string) (RuleSet, error) {
	...
	if tenantID == "" || !ruleCodePattern.MatchString(code) || name == "" {
		return RuleSet{}, apperror.Invalid("tenant_id, valid code, and name are required", nil)
	}
	now := s.now()
	value := RuleSet{ID: uuid.NewString(), TenantID: tenantID, Code: code, Name: name, Description: strings.TrimSpace(description), Status: "draft", Audit: newAudit(actorID, now)}
...
func (s *Service) UpdateRuleSet(ctx context.Context, tenantID, id, name, description, status string, expected int64) (RuleSet, error) {
	actorID, err := authorize(ctx, tenantID)
	if err != nil {
		return RuleSet{}, err
	}
	...
	value, err := s.repository.GetRuleSet(ctx, strings.TrimSpace(tenantID), strings.TrimSpace(id), "")
	if err != nil {
...
func (s *Service) GetRuleSet(ctx context.Context, tenantID, id, code string) (RuleSet, *RuleVersion, error) {
	if _, err := authorize(ctx, tenantID); err != nil {
		return RuleSet{}, nil, err
	}
	value, err := s.repository.GetRuleSet(ctx, strings.TrimSpace(tenantID), strings.TrimSpace(id), strings.ToLower(strings.TrimSpace(code)))
	if err != nil {
	...
	published, err := s.repository.GetRuleVersion(ctx, value.TenantID, value.ID, "", value.PublishedVersionNumber)
	return value, &published, translate(err)
}

func (s *Service) ListRuleSets(ctx context.Context, tenantID, status, keyword string, page, pageSize int) (Page[RuleSet], error) {
	if _, err := authorize(ctx, tenantID); err != nil {
		return Page[RuleSet]{}, err
	}
	page, pageSize = normalizePage(page, pageSize)
	items, total, err := s.repository.ListRuleSets(ctx, strings.TrimSpace(tenantID), strings.ToLower(strings.TrimSpace(status)), strings.TrimSpace(keyword), pageSize, (page-1)*pageSize)
	return Page[RuleSet]{Items: items, Total: total, Page: page, PageSize: pageSize}, translate(err)
}

func (s *Service) CreateRuleVersion(ctx context.Context, tenantID, ruleSetID, definitionJSON, idempotencyKey string) (RuleVersion, bool, error) {
	actorID, err := authorize(ctx, tenantID)
	if err != nil {
	...
	now := s.now()
	value := RuleVersion{ID: uuid.NewString(), TenantID: strings.TrimSpace(tenantID), RuleSetID: strings.TrimSpace(ruleSetID), Status: "draft", DefinitionJSON: compiled.CanonicalJSON(), Checksum: compiled.Checksum(), IdempotencyKey: strings.TrimSpace(idempotencyKey), Audit: newAudit(actorID, now)}
	created := false
...
func (s *Service) ValidateRuleVersion(ctx context.Context, tenantID, definitionJSON string) (string, []ValidationIssue, error) {
	if _, err := authorize(ctx, tenantID); err != nil {
		return "", nil, err
	}
	compiled, issues := CompileDefinition(definitionJSON)
...
func (s *Service) PublishRuleVersion(ctx context.Context, tenantID, ruleSetID, versionID string, ruleSetExpected, versionExpected int64) (RuleSet, RuleVersion, error) {
	actorID, err := authorize(ctx, tenantID)
	if err != nil {
	...
	ruleSet, err := s.repository.GetRuleSet(ctx, tenantID, ruleSetID, "")
	if err != nil {
		return RuleSet{}, RuleVersion{}, translate(err)
	}
	version, err := s.repository.GetRuleVersion(ctx, tenantID, ruleSetID, versionID, 0)
	if err != nil {
		return RuleSet{}, RuleVersion{}, translate(err)
	}
...
func (s *Service) ListRuleVersions(ctx context.Context, tenantID, ruleSetID, status string, page, pageSize int) (Page[RuleVersion], error) {
	if _, err := authorize(ctx, tenantID); err != nil {
		return Page[RuleVersion]{}, err
	}
	page, pageSize = normalizePage(page, pageSize)
	items, total, err := s.repository.ListRuleVersions(ctx, strings.TrimSpace(tenantID), strings.TrimSpace(ruleSetID), strings.ToLower(strings.TrimSpace(status)), pageSize, (page-1)*pageSize)
	return Page[RuleVersion]{Items: items, Total: total, Page: page, PageSize: pageSize}, translate(err)
}

func (s *Service) Evaluate(ctx context.Context, tenantID, ruleSetID, ruleSetCode string, versionNumber int64, factsJSON string) (Evaluation, RuleVersion, error) {
	if _, err := authorize(ctx, tenantID); err != nil {
		return Evaluation{}, RuleVersion{}, err
	}
	ruleSet, err := s.repository.GetRuleSet(ctx, strings.TrimSpace(tenantID), strings.TrimSpace(ruleSetID), strings.ToLower(strings.TrimSpace(ruleSetCode)))
	if err != nil {
	...
	version, err := s.repository.GetRuleVersion(ctx, ruleSet.TenantID, ruleSet.ID, "", versionNumber)
	if err != nil {
		return Evaluation{}, RuleVersion{}, translate(err)
	}
...
func (s *Service) addEvent(ctx context.Context, tx *sqlx.Tx, ruleSet RuleSet, version RuleVersion, actorID string, at time.Time, payload proto.Message) error {
	envelope, err := platformevents.NewEnvelope(platformevents.Metadata{EventID: uuid.NewString(), EventType: "platform.rule.v1.RuleVersionPublished", AggregateID: ruleSet.ID, AggregateType: "rule_set", TenantID: ruleSet.TenantID, SchemaVersion: 1, ActorID: actorID, OccurredAt: at}, payload)
	if err != nil {
		return err
	}
